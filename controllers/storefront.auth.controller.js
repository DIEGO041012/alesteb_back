...
exports.googleAuth = async (req, res) => {
  ...
  try {
    ...
    // 1. Verificar el access_token con Google
    let googleUser;
    try {
    ...
    if (!email) {
      return res.status(400).json({
        success: false,
        message: "No se pudo obtener el email desde Google",
    ...
    if (userRes.rowCount > 0) {
      ...
      // 3. Crear usuario nuevo (Google ya verificó el email)
      const roleRes = await client.query("SELECT id FROM roles WHERE name = 'user' LIMIT 1");
      if (roleRes.rowCount === 0) {
        await client.query("ROLLBACK");
        return res.status(500).json({
      ...
      const newUserRes = await client.query(
        `INSERT INTO users
           (email, name, cedula, is_active, is_verified, owner_admin_id, last_login)
         VALUES ($1, $2, $3, true, true, $4, NOW())
         RETURNING id, email, name, phone, cedula, city, address`,
        [
          email.toLowerCase().trim(),
          name || email.split("@")[0],
          `GOOGLE_${googleId}`, // placeholder único para cedula
          ownerAdminId,
        ]
      );
      ...
      console.log(`[STOREFRONT GOOGLE AUTH] Nuevo usuario: ${user.email} → tenant ${ownerAdminId}`);
    }
    ...
    await saveRefreshToken(client, user.id, refreshToken, "google-oauth");
    await client.query("COMMIT");

    console.log(`[STOREFRONT GOOGLE LOGIN] ${user.email} → tenant ${ownerAdminId}`);
    ...
    await client.query("ROLLBACK");
    console.error("[STOREFRONT GOOGLE AUTH ERROR]", error);
    return res.status(500).json({
      success: false,
